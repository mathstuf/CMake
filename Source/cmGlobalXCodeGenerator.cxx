                                        ...
                                        std::vector<cmLocalGenerator*>& gens)
{
  ...
  allbuild->AddSourceCMP0049(listfile);
...
//          target.GetType() == cmTarget::OBJECT_LIBRARY ||
      ...
      listfile += "/";
      listfile += "CMakeLists.txt";
      target.AddSourceCMP0049(listfile);
      }
    }
}
                                           ...
                                           targets)
{
  ...
  for(cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
    ...
    if(this->SpecialTargetEmitted(l->first))
      {
      continue;
      }
                                                ...
                                                const char* name)
{
  ...
  for (std::vector<std::string>::const_iterator currentConfig=
       ...
       currentConfig++ )
    {
    this->CreateCustomRulesMakefile(makefile,
                                    target,
                                    commands,
                                    *currentConfig,
  ...
  cdir = this->ConvertToRelativeForXCode(cdir);
  ...
  for(std::vector<cmCustomCommand>::const_iterator i = commands.begin();
    ...
    if(ccg.GetNumberOfCommands() > 0)
      ...
      if(!outputs.empty())
        ...
        std::string primaryOutput = this->ConvertToRelativeForMake(*o);
        for(++o; o != outputs.end(); ++o)
          {
          std::string currentOutput=this->ConvertToRelativeForMake(*o);
    ...
    if(ccg.GetNumberOfCommands() > 0)
      ...
      if(!outputs.empty())
        ...
        std::string primary_output =
            this->ConvertToRelativeForMake(*outputs.begin());
      ...
      for(std::vector<std::string>::const_iterator d =
        ...
        if(this->CurrentLocalGenerator
           ->GetRealDependency(*d, configName, dep))
          {
          makefileStream << "\\\n" <<
                                          ...
                                          cmXCodeObject* buildPhases)
{
  ...
  if(this->XcodeVersion > 20)
    ...
    this->CreateBuildSettings(cmtarget, buildSettings, defConfig);
    target->AddAttribute("buildSettings", buildSettings);
    }
  cmXCodeObject* dependencies =
    this->CreateObject(cmXCodeObject::OBJECT_LIST);
  ...
  if(cmtarget.GetType() == cmTarget::OBJECT_LIBRARY)
    ...
    {
    fullName = cmtarget.GetFullName(defConfig);
    }
    ...
    for(cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
      ...
      for(std::vector<cmSourceFile*>::const_iterator s = classes.begin();
        ...
        cmSourceGroup* sourceGroup =
          mf->FindSourceGroup(source, sourceGroups);
                     ...
                     generators)
{
  ...
  this->RootObject->SetId(this->GetOrCreateId(
    project_id, this->RootObject->GetId()));
  ...
  std::vector<cmXCodeObject*>& targets)
{
  cmGeneratedFileStream
    makefileStream(this->CurrentXCodeHackMakefile);
  if(!makefileStream)
    {
    ...
    for(std::map<std::string, cmXCodeObject::StringVec>::const_iterator ci
      ...
      for(cmXCodeObject::StringVec::const_iterator d = ci->second.begin();
        ...
        if(emitted.insert(*d).second)
          {
          makefileStream <<
            this->ConvertToRelativeForMake(*d) << ":\n";
          }
        }
      }
    }
