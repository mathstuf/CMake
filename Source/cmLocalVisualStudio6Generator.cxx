             ...
             std::ostream &fout, const std::string& libName)
{
  ...
  for(std::vector<const cmSourceFile *>::const_iterator sf =
    ...
    if (GetVS6TargetName(source) != libName ||
      ...
      fout << "SOURCE=" <<
        this->ConvertToOptionallyRelativeOutputPath(source) << "\n\n";
      if(!depends.empty())
        {
      ...
      if (command)
        ...
        for(std::vector<std::string>::iterator i
          ...
          if (i == this->Configurations.begin())
            {
            fout << "!IF  \"$(CFG)\" == " << *i << std::endl;
            }
          else
            {
            fout << "!ELSEIF  \"$(CFG)\" == " << *i << std::endl;
            }
  ...
  for(i = this->Configurations.begin(); i != this->Configurations.end(); ++i)
    ...
    for(std::vector<std::string>::const_iterator d =
      ...
      if(this->GetRealDependency(*d, config, dep))
        {
        fout << "\\\n\t" <<
          this->ConvertToOptionallyRelativeOutputPath(dep);
        }
      }
    fout << "\n";
    ...
    if(ccg.GetOutputs().empty())
      ...
      for(std::vector<std::string>::const_iterator o =
        ...
        fout << this->ConvertToOptionallyRelativeOutputPath(*o)
                                                  ...
                                                  const std::string& config)
{
  ...
  for(int j=0; j < 2; ++j)
    ...
    for(i = includes.begin(); i != includes.end(); ++i)
      {
      std::string tmp =
        this->ConvertToOptionallyRelativeOutputPath(*i);
      if(useShortPath)
        {
                 ...
                 std::vector<cmSourceGroup> &)
{
  ...
  if(libPath.size())
    {
    std::string lpath =
      this->ConvertToOptionallyRelativeOutputPath(libPath);
    if(lpath.size() == 0)
      {
      lpath = ".";
      }
    ...
    lpathIntDir =
      this->ConvertToOptionallyRelativeOutputPath(lpathIntDir);
  ...
  if(exePath.size())
    {
    std::string lpath =
      this->ConvertToOptionallyRelativeOutputPath(exePath);
    if(lpath.size() == 0)
      {
      lpath = ".";
      }
  ...
  for(j = libs.begin(); j != libs.end(); ++j)
    ...
    if ((target.GetType() != cmTarget::SHARED_LIBRARY
      ...
      lib = this->ConvertToOptionallyRelativeOutputPath(lib);
      libDebug =
        this->ConvertToOptionallyRelativeOutputPath(libDebug);
  ...
  if(target.GetType() == cmTarget::EXECUTABLE ||
    ...
    outputDirDebug =
        removeQuotes(this->ConvertToOptionallyRelativeOutputPath(
                       target.GetDirectory("Debug")));
    outputDirRelease =
        removeQuotes(this->ConvertToOptionallyRelativeOutputPath(
                 target.GetDirectory("Release")));
    outputDirMinSizeRel =
        removeQuotes(this->ConvertToOptionallyRelativeOutputPath(
                 target.GetDirectory("MinSizeRel")));
    outputDirRelWithDebInfo =
        removeQuotes(this->ConvertToOptionallyRelativeOutputPath(
                 target.GetDirectory("RelWithDebInfo")));
    }
  else if(target.GetType() == cmTarget::OBJECT_LIBRARY)
    {
  ...
  if(target.GetType() == cmTarget::SHARED_LIBRARY ||
    ...
    targetImplibFlagDebug +=
      this->ConvertToOptionallyRelativeOutputPath(fullPathImpDebug);
                     ...
                     std::string& options)
{
  ...
  for(std::vector<std::string>::const_iterator d = linkDirs.begin();
    ...
    if(!dir.empty())
      ...
      options += this->ConvertToOptionallyRelativeOutputPath(dir);
      options += " /LIBPATH:";
      options += this->ConvertToOptionallyRelativeOutputPath(*d);
      options += "\n";
      }
    }
  for(ItemVector::const_iterator l = linkLibs.begin();
    ...
    if(l->IsPath)
      {
      options +=
        this->ConvertToOptionallyRelativeOutputPath(l->Value);
      }
    else if (!l->Target
                ...
                std::string& options)
{
  ...
  for(std::vector<std::string>::const_iterator
    ...
    options += tool;
    options += "32 ";
    options += this->ConvertToOptionallyRelativeOutputPath(*oi);
    options += "\n";
    }
}
